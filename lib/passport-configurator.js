...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var localCallback = function(req, res, next) {
    passport.authenticate(
      name, options.fn || _.defaults({
        ...
        if (session) {
          console.log('passport-configurator: on local call back', session);
          ...
          req.logIn(user, function(err) {
            if (err) {
              if (options.failureFlash) {
                ...
                var msg = flash.message || 'LOGIN_FAILED';
  ...
  if (authType === 'local') {
    ...
    // console.log('passport-configurator:', options);
